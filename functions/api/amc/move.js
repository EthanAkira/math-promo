...
export async function onRequestPost({ request, env }) {
  ...
  const { password, level, year, variantId, fileType, to } = body || {};
  if (password !== env.AMC_UPLOAD_PASSWORD) {
    return jsonResponse({ error: 'Incorrect password.' }, { status: 401 });
  }

  const toLevel = String(to?.level || '');
  ...
  const yearEntry = levelYears && levelYears.find((entry) => entry.year === Number(year));
  ...
  const oldKey = sourceFile.key || fileKey(level, year, variantId, fileType);
  const newKey = fileKey(toLevel, toYear, toVariantId, toFileType);
  ...
  const destLevelYears = manifest[toLevel];
  let destYearEntry = destLevelYears.find((entry) => entry.year === Number(toYear));
  if (!destYearEntry) {
    destYearEntry = { year: Number(toYear), variants: [] };
    destLevelYears.push(destYearEntry);
  }
