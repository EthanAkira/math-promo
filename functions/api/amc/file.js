...
export async function onRequestGet({ request, env }) {
  ...
  if (tag && tag.access_tier === 'premium') {
    ...
    const entitled = user && await hasActiveSubscription(env.DB, user.id, 'amc');
    if (!entitled) {
...
function jsonResponse403() {
  return new Response(JSON.stringify({ error: 'A premium AMC subscription is required to access this file.' }), {
    status: 403,
