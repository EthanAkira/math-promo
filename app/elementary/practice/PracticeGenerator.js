...
export default function PracticeGenerator() {
  ...
  return <div className="worksheet-app">
    ...
    <div className={`worksheet-paper ${view === 'answers' ? 'answer-sheet' : ''}`}>
      <header className="worksheet-heading"><div className="worksheet-brand"><span className="brand-mark">{en ? 'DAILY' : '매일'}</span><strong>{en ? 'MATH PRACTICE' : '수학 연습장'}</strong></div><div className="worksheet-title"><span>{gradeLabel}</span><h2>{unitLabel} {view === 'answers' ? (en ? 'Answer Key' : '정답지') : (en ? 'Worksheet' : '문제지')}</h2><p>{unitDescription}</p></div><div className="worksheet-identity"><div><span>{en ? 'Worksheet ID' : '문제지 번호'}</span><strong>{seed}</strong><small>{en ? 'Scan the QR code to reopen this exact worksheet.' : 'QR을 스캔하면 같은 문제를 다시 열 수 있어요.'}</small></div>{qrDataUrl ? <img src={qrDataUrl} alt={`Worksheet ${seed} QR code`} /> : null}</div></header>
      ...
      <section className="problem-grid" aria-label={`${unitLabel} ${en ? 'problems' : '문제'}`}>
        {problems.map((problem) => { const isCorrect = normalizeAnswer(answers[problem.id]) === normalizeAnswer(problem.answer); const hasAnswer = answers[problem.id] !== undefined && answers[problem.id] !== ''; return <article className={`vertical-problem ${problem.kind === 'inline' ? 'inline-problem' : ''}`} key={problem.id}><span className="problem-number">{problem.id}</span><ProblemBody problem={problem} view={view} value={answers[problem.id]} checked={checked} onChange={handleAnswer} language={language} />{checked && view === 'problems' && hasAnswer ? <span className={`result-mark ${isCorrect ? 'correct' : 'wrong'}`}>{isCorrect ? (en ? 'Correct' : '맞았어요') : (en ? 'Try again' : '다시 풀기')}</span> : null}</article>; })}
      </section>
      <footer className="worksheet-footer"><span>{en ? 'Daily Math Practice' : '매일 수학 연습장'}</span><span>{seed} · {gradeLabel} · {unitLabel}</span></footer>
    </div>
