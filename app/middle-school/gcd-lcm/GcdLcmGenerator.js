...
export default function GcdLcmGenerator() {
  ...
  return <div className="worksheet-app">
    <section className="worksheet-controls no-print" aria-label={en ? 'Worksheet settings' : '문제지 설정'}>
      <div><label htmlFor="gcd-lcm-unit">{en ? 'Skill' : '문제 유형'}</label><select id="gcd-lcm-unit" value={unitId} onChange={(event) => chooseUnit(event.target.value)}>{GCD_LCM_UNITS.map((item) => <option key={item.id} value={item.id}>{localizeGcdLcmUnit(item, language)}</option>)}</select><p>{unitDescription}</p></div>
    ...
    <div className={`worksheet-paper middle-worksheet ${view === 'answers' ? 'answer-sheet' : ''}`}>
      ...
      <section className="problem-grid word-problem-grid prime-problem-grid" aria-label={`${unitLabel} ${en ? 'problems' : '문제'}`}>
        {problems.map((item) => {
          ...
          return <article className="vertical-problem word-problem prime-problem" key={item.id}><span className="problem-number">{item.id}</span><div className="word-calculation"><p>{en && item.promptEn ? item.promptEn : item.prompt}</p>{item.expression ? <strong className="word-expression font-mono"><PowerText value={item.expression} /></strong> : null}<div className="word-answer"><span>{en ? 'Answer' : '답'}</span><span className="inline-answer">{view === 'answers' ? <strong><PowerText value={item.answer} /></strong> : <><input aria-label={en ? `Answer ${item.id}` : `${item.id}번 답`} value={value} onChange={(event) => changeAnswer(item.id, event.target.value)} className={checked && value ? (isCorrect ? 'correct' : 'wrong') : ''} /><span className="print-answer-space" aria-hidden="true" /></>}</span>{item.answerSuffix ? <em>{item.answerSuffix}</em> : null}</div></div>{checked && view === 'problems' && value ? <span className={`result-mark ${isCorrect ? 'correct' : 'wrong'}`}>{isCorrect ? (en ? 'Correct' : '맞았어요') : (en ? 'Try again' : '다시 풀기')}</span> : null}</article>;
        })}
      </section>
    ...
    {view === 'problems' ? <section className="grading-panel no-print"><div><strong>{en ? 'Solve directly on a tablet' : '태블릿으로 바로 풀기'}</strong><p>{en ? 'Separate multiple answers with commas. Use ^ for exponents.' : '답이 여러 개이면 쉼표로 구분하세요. 거듭제곱은 2^3처럼 입력합니다.'}</p></div><button className="button button-primary" onClick={() => setChecked(true)}>{en ? 'Check answers' : '채점하기'}</button>{checked ? <strong className="score">{en ? `${correctCount} of 20 correct` : `20문제 중 ${correctCount}문제 정답`}</strong> : null}</section> : null}
  </div>;
}
