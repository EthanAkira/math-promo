...
export default function BasicFiguresGenerator() {
  ...
  const [unitId, setUnitId] = useState(BASIC_FIGURE_UNITS[0].id);
  ...
  useEffect(() => {
    ...
    const initialUnit = findBasicFigureUnit(params.get('unit')).id;
  ...
  const unitLabel = localizeBasicFigureUnit(unit, language);
  const unitDescription = localizeBasicFigureUnit(unit, language, 'description');

  return <div className="worksheet-app">
    <section className="worksheet-controls no-print" aria-label={tr(language, 'worksheetSettings')}>
      <div><label htmlFor="basic-figures-unit">{tr(language, 'skill')}</label><select id="basic-figures-unit" value={unitId} onChange={(event) => chooseUnit(event.target.value)}>{BASIC_FIGURE_UNITS.map((item) => <option key={item.id} value={item.id}>{localizeBasicFigureUnit(item, language)}</option>)}</select><p>{unitDescription}</p></div>
